...
int i = 0, k=0, valor_inst = 0, numero_muestrasAC = 1200, numero_muestrasDC = 100, rango_actual = 3;
float lectura = 0, volt_value = 0, data_to_save[5], data_read[5];
...
void config(){
    ...
    esp_rom_gpio_pad_select_gpio(pin3k9);
    gpio_set_direction(pin3k9,GPIO_MODE_OUTPUT);
...
void autorango(float valor_actual){
    ...
    if(valor_actual<min_adc){
        switch(rango_actual){
            ...
            case 3:{
                lcd_set_cursor(0, 0);
                lcd_send_string("Tensión muy baja ");
                break;
            }
        }
    } 
...
void lectura_tensionDC(){
    ...
    sprintf(buffer_display_inferior, "      %.2f      ",volt_value );
    lcd_set_cursor(0, 1);
    lcd_send_string(buffer_display_inferior);

    vTaskDelay(5 / portTICK_PERIOD_MS); // Espera antes de la próxima lectura   
}
...
void lectura_tensionAC(){
    ...
    volt_value = ((volt_value * 3.3)/4096)+0.1;
