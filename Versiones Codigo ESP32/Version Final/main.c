...
int i = 0, k=0, numero_muestrasAC = 6000, numero_muestrasDC = 100, rango_actual = 3,esDC, mostrarIP=0;
float lectura = 0, valor_inst = 0, volt_value = 0,data_to_save[5] = {0,0,0,0,0}, data_read[5] = {0,1,2,0,0};
char matriz_char[5][11], buffer_display_superior[20], buffer_display_inferior[20],ip[16];
...
esp_err_t hello_get_handler(httpd_req_t *req) {
    ...
    strcpy(response, "Valores medidos:<br>");

    // Agrega cada valor de la matriz a la respuesta
    for (int i = 0; i < 5; i++) {
        strcat(response, matriz_char[i]);
        if(esDC==0)strcat(response," AC<br>");
        if(esDC==1)strcat(response," DC<br>");
    }
...
float calculo_voltaje_DC(float dato){
    ...
    switch(rango_actual){
        case 1:{
            dato=-5441.24581*pow(dato,5)+8441.31484*pow(dato,4)-5114.67702*pow(dato,3)+1509.06307*pow(dato,2)-121.6047*dato+4.129202;  
            break;
        }
        case 2: {
            dato=-0.098587*pow(dato,5)+0.6014112*pow(dato,4)-1.292964*pow(dato,3)+1.129073*pow(dato,2)+6.1041049*dato-0.5816366;  
            break;
        }
        case 3:{
...
float calculo_voltaje_AC(float dato){
    ...
    switch(rango_actual){
        ...
        case 2: {
            dato=6.3398*pow(dato,5)-20.58217*pow(dato,4)+24.95388*pow(dato,3)-14.207387*pow(dato,2)+10.362489*dato-0.52505;
            break;
        }
        case 3:{
            dato=0.05*pow(dato,4)-0.18*pow(dato,3)+0.25*pow(dato,2)+0.36*dato; 
            break;
        }
    }
    return dato;
...
void lectura_tensionDC(){
    ...
    if(volt_value<0.1){
        lcd_set_cursor(0, 0);
        lcd_send_string("Ingrese tension ");
        lcd_set_cursor(0, 1);
        lcd_send_string("--------------- ");
    }else{
    lcd_set_cursor(0, 0);
...
while (1) {
        ...
        if(guardar_datos==true){
            ...
            guardar_datos=false;
            lcd_set_cursor(0, 0);
            lcd_send_string("      Dato      ");
            lcd_set_cursor(0, 1);
            lcd_send_string("    guardado    ");
            vTaskDelay(2000 / portTICK_PERIOD_MS);
        }

        if(deteccionAC()==1 && mostrarIP==0){
            lectura_tensionAC();
        }
        else{
            if(mostrarIP==0){
